...
if( opt$pli.source == "exac" ) {
	...
	pli.score <- read.table("fordist_cleaned_exac_r03_march16_z_pli_rec_null_data.txt", header=T, sep="\t")
	...
	pli.score <- read.table("gnomad.v2.1.1.lof_metrics.by_gene.txt", header=T, sep="\t")
...
ssc.dnm.ihv <- read.table("Ratio_Singleton_DeNovo.txt", header=T, sep="\t")
...
adjust_group <- function( group_interval, group_prefix ) {
	...
	system(paste("Rscript trapd_base_QQ.R", "--prefix", paste( group_prefix, sep="") ) )
...
system(paste("Rscript trapd_base_QQ.R", "--prefix", "Final.Result"))
invisible(file.remove( pLI.stratified.tempFile ))
