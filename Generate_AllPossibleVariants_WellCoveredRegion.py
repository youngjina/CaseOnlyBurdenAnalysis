...
def doAllChromosomes(cohort, mutationType):
	...
	for chromosome in chromosomes:
		...
		coveredPositionsFileName = "Coverage_Summary_{cohort}_min-coverage_10/Covered_chr_pos/chr{chr}_Covered_90_percent_Positions_{cohort}_min-coverage_10_chr_pos.txt".format(cohort = cohort, chr = chromosome)

		caddFilename = "absent_{mutationType}/absent_{mutationType}.chr{chr}.txt".format(chr = chromosome, mutationType = mutationType)

		outputDirectory = "Coverage_Summary_{cohort}_min-coverage_10/Absent_gnomAD_{mutationType}_variantID/".format(mutationType = mutationType, cohort = cohort)
